@ ------------------------------------------------------------
@ LAB ONE: ARRAY PRINTER
@
@ AUTHOR: LUCAS THELEN
@
@ PURPOSE: Prints two arrays, sums and prints a third, and
@ then prints a filtered third array
@
@ COMPILE AND RUN: 
@ `as -o program.o program && gcc -o program program.o`
@ ./program
@ ------------------------------------------------------------
...
filter_arr_loop_start:
	...
	@ But wait! If r0 is zero, we need to continue the loop now!
	cmp	r0, #0
	beq	filter_arr_loop_end	

	@ If it's not zero, we haven't branched, and now we can check if they have the same sign
	lsr	r7, r0, #31	@ Isolate sign bit with logical shift
	lsr	r8, r5, #31
	cmp	r7, r8		@ Compare the sign bit
...
welcome_six:	.asciz "Filtered Array(Sign of item matches sign of input): "
...
arr_two:	.word -10, -9, -8, -7, -5, -6, -4, -3, -2, -1
